...
public:
    ...
    void Compute(OpKernelContext *context) override
    {
        ...
        if (device == kDLCPU)
        {
            memcpy(output_flat.data(), dlm_tensor->dl_tensor.data, num_elements_ * (dlm_tensor->dl_tensor.dtype.bits) / 8);
        }
        else
        {
            cudaMemcpy(output_flat.data(), dlm_tensor->dl_tensor.data, num_elements_ * (dlm_tensor->dl_tensor.dtype.bits) / 8, cudaMemcpyDeviceToDevice);
        }
